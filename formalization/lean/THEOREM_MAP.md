...
| Rank-threshold projected rank-range bounds | `PathCompressionDigestion/SourceProjection.lean` | `RawCompressionPath.ProjectedPathSegment.edgeCost_le_rankBound_of_nonroot`, `RawDissection.bottomParent_rank_lt_of_not_root`, `RawDissection.topParent_rank_lt_of_not_root`, `RankThresholdDissection.topParent_shiftedRank_lt_of_not_root`, `RawCompressionStep.bottomProjectedStep_consumableCost_le_rankThreshold`, `RawCompressionStep.topProjectedStep_consumableCost_le_rankThreshold`, `RawCompressionExecution.rankThresholdBottomProjectedExecution_consumableCost_le_threshold_mul_chargedCount`, `RawCompressionExecution.rankThresholdTopProjectedExecution_consumableCost_le_shiftedRank_mul_chargedCount` | Proved rank-range sub-obligations; not enough for full recurrence consumption | Charged bottom projections have total consumable cost at most `s * Cb.chargedCount`; charged top projections have total consumable cost at most `(r - s - 1) * Ct.chargedCount`. The package still needs the sharper Seidel--Sharir recurrence-consumed bounds with coefficients `k + 1` and `k` plus side-cardinality budgets. |
