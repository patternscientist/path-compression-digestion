...
| Concrete source-model skeleton | `PathCompressionDigestion/ConcreteSourceModel.lean` | `RawRankedForest`, `RawRankedForest.highRankFinset`, `RawRankedForest.HasRankThresholdPacking`, `RawCompressionPath`, `RawCompressionStep`, `RawCompressionExecution`, `RawCompressionExecution.HasLegacyBaseRankAccounting`, `RawCompressionExecution.HasRankThresholdPacking`, `RawCompressionExecution.HasBaseRankAccounting`, `topDownCost`, `topDownCost_le_base_budget`, `topDown_base_bound`, `topDown_base_sourceBound`, `topDownShiftStepTarget`, `topDown_sourceModel_of_shift`, `sourceRecurrence_topDownCost_of_shift`, `paper_finite_bound_topDownCost_of_shift` | Base obligation proved for base-accounted executions; faithful validity now includes direct rank-threshold packing; full pipeline conditional on shift; shift open | Defines finite ranked-forest/path-compression execution objects and a concrete `SourceCostFamily`. The base bound `SourceBound topDownCost 0 (J 0)` is proved from explicit base-rank-accounting certificates, now strengthened with direct rank-threshold packing. Deriving those certificates from raw step semantics and proving the Seidel--Sharir shift remain open. Conditional wrappers show the full A-shaped pipeline follows from the shift theorem alone. |
| Source dissection/projection packing repair | `PathCompressionDigestion/SourceDissection.lean`, `PathCompressionDigestion/SourceProjection.lean` | `RankThresholdDissection.topPacking_of_top_card_mul_pow_le`, `RankThresholdDissection.topPacking_of_rankThresholdPacking`, `RawCompressionExecution.rankThresholdDissectionFamily_topPacking`, `RawCompressionExecution.exists_legacyValidExecution_without_rankThresholdTopPacking_current_model`, `RawCompressionExecution.not_exists_validExecution_without_rankThresholdTopPacking_current_model` | Top-packing model defect repaired; recurrence consumable simulations still open | Direct rank-threshold packing now constructs the `TopPacking` witness required by `RankThresholdLogConsumableBounds`. The old one-vertex high-rank root obstruction is preserved only for `IsLegacyValidWithoutRankPacking`, and the faithful `IsValid` version is ruled out. |
| Rank-threshold projected rank-range and bottom-local packing bounds | `PathCompressionDigestion/SourceDissection.lean`, `PathCompressionDigestion/SourceProjection.lean` | `RankThresholdDissection.bottom_highRank_card_mul_pow_le_bottom_card`, `RawCompressionExecution.rankThresholdDissectionFamily_bottom_highRank_card_mul_pow_le_bottomFinset_card`, `RawCompressionPath.ProjectedPathSegment.edgeCost_le_rankBound_of_nonroot`, `RawDissection.bottomParent_rank_lt_of_not_root`, `RawDissection.topParent_rank_lt_of_not_root`, `RankThresholdDissection.topParent_shiftedRank_lt_of_not_root`, `RawCompressionStep.bottomProjectedStep_consumableCost_le_rankThreshold`, `RawCompressionStep.topProjectedStep_consumableCost_le_rankThreshold`, `RawCompressionExecution.rankThresholdBottomProjectedExecution_consumableCost_le_threshold_mul_chargedCount`, `RawCompressionExecution.rankThresholdTopProjectedExecution_consumableCost_le_shiftedRank_mul_chargedCount` | Proved structural sub-obligations; not enough for full recurrence consumption | Direct global rank packing now localizes to the rank-threshold bottom side. Charged bottom projections have total consumable cost at most `s * Cb.chargedCount`; charged top projections have total consumable cost at most `(r - s - 1) * Ct.chargedCount`. The package still needs the sharper Seidel--Sharir recurrence-consumed bounds with coefficients `k + 1` and `k` plus side-cardinality budgets. |
...
- Concrete source-model skeleton with finite ranked forests, direct
  rank-threshold packing, top-down compression paths/steps/executions, and
  `topDownCost`; the base source obligation `topDown_base_sourceBound` is
  proved for base-accounted executions, and the finite paper bound is
  available conditional on the remaining shift theorem.
- Rank-threshold top packing is now derived from the direct packing invariant;
  the old one-vertex high-rank root obstruction is preserved only for the
  explicit legacy-without-packing predicate.
- Rank-threshold projected step rank-range bounds are now proved: bottom
  consumable projected cost is bounded by `s` per charged bottom projection,
  and top consumable projected cost is bounded by `r - s - 1` per charged top
  projection.
- Direct rank-threshold packing is now proved to localize to the bottom side
  of a rank-threshold dissection, in the exact cardinality form needed by a
  future bottom restricted-execution simulation.
